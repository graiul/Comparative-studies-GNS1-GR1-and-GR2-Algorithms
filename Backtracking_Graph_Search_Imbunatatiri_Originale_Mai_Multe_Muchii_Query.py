...
def is_joinable(data_edge_to_be_joined, partial_solution, data_graph, query_edges_dict_input):
            ...
            if list(query_edges_dict.items())[0][1][0] == data_edge_to_be_joined_node_0_label or list(query_edges_dict.items())[0][1][0] == data_edge_to_be_joined_node_1_label:
                ...
                if list(query_edges_dict.items())[0][1][1] == data_edge_to_be_joined_node_1_label or list(query_edges_dict.items())[0][1][1] == data_edge_to_be_joined_node_0_label:
                    ...
                    # if data_edge_to_be_joined not in positions[0]:
                        ...
                        # print("Log for position 0: " + str(positions[pos]))
                    ...
                    # if data_edge_to_be_joined not in positions[1]:
                        ...
                        if aux not in complete_solutions:
                            ...
                            for e in partial_solution:
                                ...
                                # print("     " + Fore.GREEN + Style.BRIGHT + "Positions log after appending second edge: " + str(list(positions.items())) + Style.RESET_ALL)
                                # print()
                else:
                    # print("     Data edge is not valid for this.")
                    pass
    ...
    if len(partial_solution) == 4:
        if data_edge_to_be_joined not in partial_solution:
            if list(query_edges_dict.items())[4][1][0] == data_edge_to_be_joined_node_0_label or list(query_edges_dict.items())[4][1][0] == data_edge_to_be_joined_node_1_label:
                if list(query_edges_dict.items())[4][1][1] == data_edge_to_be_joined_node_1_label or list(query_edges_dict.items())[4][1][1] == data_edge_to_be_joined_node_0_label:
                    aux = copy.deepcopy(partial_solution)
                    aux.append(data_edge_to_be_joined)
                    pos = aux.index(aux[-1])
                    if aux not in complete_solutions:
                        finder = EdgeFinderTool(data_edge_to_be_joined, positions[4])
                        found = finder.edge_found()
                        if found is False:
                            for e in partial_solution:
                                if data_graph.has_edge(e[0], data_edge_to_be_joined[0]) or \
                                    ...
                                    if found2 is False:
                                        positions[pos].append(aux[-1])
                                    print("     " + Fore.GREEN + Style.BRIGHT + "Positions log after appending FIFTH edge: " + str(list(positions.items())) + Style.RESET_ALL)
                                    print()
                                    print("----------------------------------------------------------")
                else:
                    # print("     Data edge is not valid for this.")
...
def subgraph_search(partial_solution, query_graph_dict, data_graph):
    # print()
    ...
    if len(partial_solution) == len(list(query_graph_dict.items())):
        if partial_solution not in complete_solutions:
            ...
            for pe in partial_solution:
                print("Partial solution edge: " + str(pe))
                print("Partial solution edge nodes labels: ")
                print([data_graph.nodes[pe[0]]['label'], data_graph.nodes[pe[1]]['label']])
            print()
            ...
            # current_edge = copy.deepcopy(partial_solution[-1])
            # current_node = []
            ...
            subgraph_search(partial_solution, query_graph_dict, data_graph)
        ...
        if candidate is None:  # go back a position with restore position()
            ...
            if len(partial_solution) == 1: # poz 0 = [x]
                # current_edge = copy.deepcopy(partial_solution[0])
                # positions[1] = []
                positions[2] = [] # poz 1 = []
            ...
            if len(partial_solution) == 3: # poz 0 = [x], poz 1 = [y]
                # current_edge = copy.deepcopy(partial_solution[-2])
                positions[4] = []
...
p_solution = []
complete_solutions = []
# Aici este numarul de muchii pe care il poate avea un graf query acceptat de algoritm.
positions = OrderedDict().fromkeys([0,1,2,3,4])
positions[0] = []
positions[1] = []
positions[2] = []
positions[3] = []
